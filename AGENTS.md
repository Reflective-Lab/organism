...
| Layer | Technology |
|---|---|
| System logic | Rust (Edition 2024, rust-version 1.96) |
| Converge contract | `converge-pack`, `converge-kernel`, `converge-provider-api`, optional `converge-model` / `converge-client` (v3.7.6) |
| Task runner | just |
...
- Before building a core capability, check `~/dev/reflective/bedrock-platform/converge/CAPABILITIES.md` — Converge provides optimization solvers, knowledge base, policy engine, analytics/ML, LLM providers, tool integration, experience store, object storage.
...
Read `MILESTONES.md` at the start of every session. Scope all work to the current milestone. See `~/dev/reflective/bedrock-platform/EPIC.md` for the strategic context (Organism = E2) and `~/dev/reflective/bedrock-platform/MILESTONES.md` for the cross-project rollup.
