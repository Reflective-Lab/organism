...
- **`organism-domain` path** — moved to
  `~/dev/reflective/mosaic-extensions/atelier-showcase/crates/organism-domain` (workspace
...
- **`organism-intelligence::linkedin`** module — extracted to
  `embassy-linkedin` (`~/dev/reflective/mosaic-extensions/embassy-ports/crates/linkedin`).
...
- The loan-application example graduated out of Organism into
  `~/dev/reflective/marquee-apps/loan-application`, keeping lending vocabulary and underwriting
